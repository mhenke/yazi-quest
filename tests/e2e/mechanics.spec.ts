...
test.describe('Game Mechanics & Failures', () => {
  ...
  test.describe('Level 2 Mechanics', () => {
    ...
    test('Shell Collapse: Deleting critical system directory triggers Game Over', async ({ page }) => {
      // Navigate to Root
  ...
  test.describe('Level 11 Mechanics', () => {
    test.beforeEach(async ({ page }) => {
      await startLevel(page, 11, { intro: false });
    });

    test('L11 Honeypot: Selecting recent service does not complete task', async ({ page }) => {
  ...
  test.describe('Level 1 Mechanics', () => {
    test.beforeEach(async ({ page }) => {
