...
// Added missing import for sortNodes
...
export const LEVELS: Level[] = [
  ...
  },
  {
    ...
    description: "ANOMALY DETECTED. A heuristic scanner is sweeping the sector, mirroring your neural signatures. It creates 'ghost' artifacts that capture your metadata. You must standing at the root of the system to triangulate the scanner's current collection buffer—it's hidden deep in the hierarchy. Expose it, and terminate the connection.",
    initialPath: null,
    hint: "1. Go to root partition (gr). 2. Activate recursive search (z), find 'ghost', and jump to it. z will automatically highlight the target. 3. identify anomaly via Size Sort (,s). 4. Delete it (d).",
    coreSkill: "FZF Recursive Find (z) & Sort",
    environmentalClue: "TARGET: ghost_process.pid | METHOD: Recursive search (z) -> Sort diagnostic (,s) | ACTION: Delete",
    successMessage: "FORENSIC MIRROR TERMINATED. CONNECTION SECURED.",
    buildsOn: [2, 5, 8],
    leadsTo: [14, 16],
    timeLimit: 120,
    efficiencyTip: "FZF (z) searches deep hierarchies recursively. From root, it can find any file instantly if you know the signature.",
    onEnter: (fs: FileNode) => {
      const tmp = findNodeByName(fs, "tmp");
      if (tmp && tmp.children) {
        // Remove any old ones
        tmp.children = tmp.children.filter(c => c.name !== "ghost_process.pid");
        // Ensure ghost_process.pid is deep and anomalous
        const now = Date.now();
        const cache = tmp.children.find(c => c.name === "cache");
        if (cache && cache.children) {
            const ghost = { 
              id: generateId(), 
              name: "ghost_process.pid", 
              type: "file", 
              content: "0x".repeat(15000), // Massive buffer
              parentId: cache.id,
              modifiedAt: now + 5000, 
              createdAt: now
            };
            cache.children.push(ghost as FileNode);
        }
      }
      return fs;
    },
    tasks: [
      {
        id: "nav-root",
        description: "Infiltrate root partition (gr)",
        check: (state: GameState) => state.currentPath.length === 1 && state.currentPath[0] === "root",
        completed: false
      },
      {
        id: "fzf-ghost",
        description: "Activate recursive search (z), locate 'ghost', and jump to its sector",
        check: (state: GameState) => {
          const currentDir = getNodeByPath(state.fs, state.currentPath);
          const hasGhost = currentDir?.children?.some(c => c.name === "ghost_process.pid");
          const isHighlighted = currentDir?.children && sortNodes(currentDir.children.filter(c => {
             if (!state.showHidden && c.name.startsWith('.')) return false;
             const filter = state.filters[currentDir.id] || '';
             return !filter || c.name.toLowerCase().includes(filter.toLowerCase());
          }), state.sortBy, state.sortDirection)[state.cursorIndex]?.name === "ghost_process.pid";
          return !!hasGhost && !!isHighlighted;
        },
        completed: false
      },
      {
        id: "sort-verify",
        description: "Triangulate anomaly: identify via Size Sort (,s)",
        check: (state: GameState) => state.sortBy === "size",
        completed: false
      },
      {
        id: "purge-ghost",
        description: "Terminate the forensic mirror: 'ghost_process.pid' (d)",
        check: (state: GameState) => {
          const ghost = findNodeByName(state.fs, "ghost_process.pid");
          return !ghost;
        },
        completed: false
      }
