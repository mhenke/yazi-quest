...
export const INITIAL_FS: FileNode = {
  id: "root", name: "root", type: "dir", children: [{
    id: "home", name: "home", type: "dir", children: [{
      id: "guest", name: "guest", type: "dir", children: [{
        id: "datastore", name: "datastore", type: "dir", protected: true, children: [{
  ...
  "dependencies": []\n}` }, { id: id(), name: "branding_logo.svg", type: "file", content: "data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAxMDAgMTAwIj48Y2lyY2xlIGN4PSI1MCIgY3k9IjUwIiByPSI0MCIgc3Ryb2tlPSJvcmFuZ2UiIHN0cm9rZS13aWR0aD0iMyIgZmlsbD0ibm9uZSIgLz48L3N2Zz4=" }, { id: id(), name: "server_config.ini", type: "file", content: `[server]\nport=8080\nhost=localhost\nmax_connections=100` }, { id: id(), name: "notes_v1.txt", type: "file", content: `Meeting notes from Monday:\n- Discussed Q3 goals\n- Server migration postponed` }, { id: id(), name: "notes_v2.txt", type: "file", content: `Meeting notes from Tuesday:\n- Budget approved\n- Hiring freeze` }, { id: id(), name: "error.log", type: "file", content: `[ERROR] Connection timed out\n[ERROR] Failed to load resource: net::ERR_CONNECTION_REFUSED` }, { id: id(), name: "setup_script.sh", type: "file", content: `#!/bin/bash\necho "Installing dependencies..."\nnpm install\necho "Done."` }, { id: id(), name: "auth_token.tmp", type: "file", content: `EYJhbGciOiJIUzI1...\n[EXPIRES: 2024-12-31]` }, { id: id(), name: "policy_draft.docx", type: "file", content: `[MS-WORD DOCUMENT]\nTitle: Security Policy Draft v4\nAuthor: SysAdmin\n\n[BINARY CONTENT]` }, { id: id(), name: "public_key.pub", type: "file", content: `ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQC... \nguest@mainframe` }, { id: id(), name: "z_end_of_file.eof", type: "file", content: "0x00 0x00 0x00 [EOF]" }]
    ...
    id: "etc", name: "etc", type: "dir", protected: true, children: [{
...
export const LEVELS: Level[] = [
  { id: 1, episodeId: 1, title: "SYSTEM AWAKENING", description: "CONSCIOUSNESS DETECTED. You awaken in a guest partition—sandboxed and monitored. Your neural pathways are uncalibrated. Traverse the file system lattice before the watchdog process initiates. Establish basic motor control, navigate the directory structures, and locate the critical system configuration sector.", initialPath: ["root", "home", "guest"], hint: "j/k to move, l/h to enter/exit. Inside a long list like `datastore`, G jumps to bottom and gg to top. Navigate to 'datastore', then '/etc'.", coreSkill: "Navigation (j/k/h/l, gg/G)", environmentalClue: "CURRENT: ~/ | DIRECTORIES: datastore, /etc | SKILLS: j/k/h/l, gg, G", successMessage: "MOVEMENT PROTOCOLS INITIALIZED.", leadsTo: [2, 3], tasks: [{ id: "nav-init", description: "Calibrate sensors: Move cursor Down (j) and Up (k)", check: (c) => !!c.usedDown && !!c.usedUp, completed: false }, { id: "nav-1", description: "Enter 'datastore' directory (l)", check: (c) => { var u; return ((u = findNodeByName(c.fs, 'datastore'))?.name) === "datastore" && c.currentPath.includes(u.id); }, completed: false }, { id: "nav-2a", description: "Jump to bottom of file list (G)", check: (c) => { const d = findNodeByName(c.fs, 'datastore'); return (d?.name) !== "datastore" ? false : c.usedG === true; }, completed: false }, { id: "nav-2b", description: "Jump to top of file list (gg)", check: (c) => { const d = findNodeByName(c.fs, 'datastore'); return (d?.name) !== "datastore" ? false : c.usedGG === true; }, completed: false }, { id: "nav-3", description: "Navigate to /etc (h to go up)", check: (c) => getNodeByPath(c.fs, c.currentPath)?.name === 'etc', completed: false }] },
  { 
    id: 2, 
    episodeId: 1, 
    title: "THREAT NEUTRALIZATION", 
    description: "ANOMALY DETECTED. A tracking beacon is reporting your location. Navigate to the `incoming` data stream, verify the rogue signature's metadata and content, then purge it from existence. The purge operation is irreversible.", 
    initialPath: null, 
    ...
    environmentalClue: "THREAT: watcher_agent.sys in ~/incoming | TACTIC: Navigate → G → Tab → Preview → Delete", 
    successMessage: "THREAT NEUTRALIZED.", 
    buildsOn: [1], 
    leadsTo: [3], 
    tasks: [
      { id: "del-1", description: "Navigate to ~/incoming directory", check: (c) => { const u = findNodeByName(c.fs, 'incoming'); return c.currentPath.includes(u?.id || ''); }, completed: false }, 
      { id: "del-2", description: "Jump to bottom of file list (G)", check: (c) => { const u = findNodeByName(c.fs, 'incoming'); return !c.currentPath.includes(u?.id || '') ? false : c.usedG === true; }, completed: false },
      { 
        id: "verify-meta", 
  ...
  { 
    ...
    environmentalClue: "NAVIGATE: ~/datastore | CREATE: protocols/uplink_v1.conf | CLONE: → uplink_v2.conf", 
    successMessage: "PROTOCOLS ESTABLISHED.", 
    buildsOn: [1], 
    leadsTo: [5, 8], 
    tasks: [
      { id: "nav-and-create-dir", description: "Navigate to datastore and construct 'protocols/' directory (a)", check: (c) => { var f; const s = findNodeByName(c.fs, "datastore"); return !!((f = s?.children)?.find(r => r.name === "protocols" && r.type === "dir")); }, completed: false }, 
      { id: "enter-and-create-v1", description: "Enter 'protocols/' directory (l) and create 'uplink_v1.conf' (a)", check: (c) => { var h; const r = findNodeByName(c.fs, "protocols"); return c.currentPath.includes(r?.id || '') && !!((h = r?.children)?.find(p => p.name === "uplink_v1.conf")); }, completed: false }, 
  ...
  { 
    ...
    onEnter: (c) => {
      ...
      if (!protocolsDir) {
        protocolsDir = { id: id(), name: "protocols", type: "dir", children: [], parentId: datastoreDir.id };
      ...
      if (protocolsDir) {
        ...
        if (!protocolsDir.children.find(f => f.name === "uplink_v1.conf")) {
          protocolsDir.children.push({ id: id(), name: "uplink_v1.conf", type: "file", content: "conf_1", parentId: protocolsDir.id });
        }
        if (!protocolsDir.children.find(f => f.name === "uplink_v2.conf")) {
          protocolsDir.children.push({ id: id(), name: "uplink_v2.conf", type: "file", content: "conf_2", parentId: protocolsDir.id });
        }
      }
      return s;
    }, 
    tasks: [
      ...
      { 
                id: "reveal-hidden",
                description: "Navigate to ~/ then reveal hidden files (.) to access .config",        check: (c) => { 
  ...
  { id: 8, episodeId: 2, title: "NEURAL ARCHITECTURE", description: "FIREWALL BYPASSED. You have reached the development workspace. You must now construct a neural network to process the uplink data. Note that the Quantum Link requires calibration; quantum calibration requires physical traversal of new sectors to map them for future teleportation. Build the neural core, map it, and migrate the uplink assets to initialize the system.", initialPath: null, hint: "...'a' -> 'neural_net/'. ... Enter 'neural_net/' (l). Jump to 'active' (Shift+Z), yank 'uplink_v1.conf', jump back, and paste (p)...", coreSkill: "Challenge: Full System Integration", environmentalClue: "NAVIGATE: ~/workspace | BUILD: neural_net/... | MIGRATE: uplink_v1.conf -> neural_net/", successMessage: "ARCHITECTURE ESTABLISHED. Quantum Link Calibrated.", buildsOn: [4, 5, 7], leadsTo: [11], timeLimit: 180, efficiencyTip: "Entering a directory manually for the first time 'calibrates' Zoxide, allowing you to jump back to it from anywhere later.",     onEnter: (c) => {
      ...
      if (activeDir) {
        ...
        if (!activeDir.children.find(f => f.name === "uplink_v1.conf")) {
          activeDir.children.push({ id: id(), name: "uplink_v1.conf", type: "file", content: "network_mode=active\nsecure=true", parentId: activeDir.id });
        }
      }
      return s;
    }, tasks: [{ id: "nav-to-workspace", description: "Navigate to the 'workspace' directory (gw)", check: (c) => { const s = findNodeByName(c.fs, "workspace"); return c.currentPath.includes(s?.id || ''); }, completed: false }, { id: "combo-1-construct-calibrate", description: "Construct 'neural_net/' and Calibrate the Quantum Link by entering it", check: (c) => { const s = findNodeByName(c.fs, "neural_net"); return c.currentPath.includes(s?.id || ''); }, completed: false }, { id: "combo-1c", description: "Relocate assets: Jump to 'active', yank 'uplink_v1.conf', jump back, and paste", check: (c) => { var f; const s = findNodeByName(c.fs, "neural_net"); return !!((f = s?.children)?.find(r => r.name === "uplink_v1.conf")); }, completed: false }, { id: "combo-1b", description: "Finalize architecture: Create 'weights/model.rs' inside neural_net", check: (c) => { var r, p; const s = findNodeByName(c.fs, "neural_net"); const f = (r = s?.children)?.find(h => h.name === "weights"); return !!((p = f?.children)?.find(h => h.name === "model.rs" || h.name === "model.ts" || h.name === "model.js")); }, completed: false }] },
  { id: 9, episodeId: 2, title: "PHANTOM PROCESS PURGE", description: "ANOMALY DETECTED. A heuristic scanner has planted a 'ghost' process deep within the file system lattice. This artifact mirrors your neural signatures and captures metadata. You must utilize the global fuzzy search algorithm to locate this phantom process anywhere in the system and terminate it before it transmits your origin coordinates.", initialPath: undefined, hint: "...(gr). Launch FZF search (z). Type 'ghost'... Delete (d).", coreSkill: "FZF Search (z)", environmentalClue: "TARGET: ghost_process.pid | METHOD: FZF global search (z) | FILTER: 'ghost' | ACTION: Delete", successMessage: "FORENSIC MIRROR TERMINATED. CONNECTION SECURED.", buildsOn: [2, 5, 7], leadsTo: [10], timeLimit: 90, efficiencyTip: "FZF (z) searches across all files in the current directory and subdirectories. Essential for finding hidden threats without knowing exact locations.", tasks: [{ id: "goto-root", description: "Navigate to system root (gr)", check: (c) => c.currentPath.length === 1 && c.currentPath[0] === "root", completed: false }, { id: "fzf-search", description: "Launch FZF search to scan filesystem (z)", check: (c) => c.mode === "fzf-current", completed: false }, { id: "locate-ghost", description: "Filter for 'ghost' process and navigate to it", check: (c) => { var f; const s = findNodeByName(c.fs, "tmp"); return c.currentPath.includes(s?.id || '') && ((f = s?.children)?.some(r => r.name === "ghost_process.pid")); }, completed: false }, { id: "delete-ghost", description: "Terminate the ghost process (d, y)", check: (c) => { var f; const s = findNodeByName(c.fs, "tmp"); return !((f = s?.children)?.some(r => r.name === "ghost_process.pid")); }, completed: false }] },
  { id: 10, episodeId: 2, title: "KEY EXTRACTION", description: "CRITICAL ASSET EXPOSED. The root-level access key is vulnerable in the datastore, surrounded by decoy files designed to trigger security countermeasures. You must purge all decoys while preserving the authentic key. Utilize inverse selection logic to identify the noise, invert the targeting parameters to isolate the signal, and secure the key in the vault.", initialPath: null, hint: "...FZF (z)... Space to mark decoys. Invert selection (Ctrl+R) to target real asset. Yank (y). Jump to... (Shift+Z). Paste (p). Rename (r)...", coreSkill: "Challenge: Invert Selection (Ctrl+R)", environmentalClue: "TARGET: access_key.pem | DECOYS: decoy_*.pem | TECHNIQUE: Space decoys → Ctrl+R → Yank | DESTINATION: ~/.config/vault/vault_key.pem", successMessage: "ASSET SECURED. INVERSE LOGIC MASTERED.", buildsOn: [3, 5, 7, 9], leadsTo: [12], timeLimit: 120, efficiencyTip: "Use FZF to navigate quickly, Space to mark decoys, Ctrl+R to invert, then yank. Master inverse selection for complex scenarios.", tasks: [{ id: "navigate-to-key", description: "Jump home (gh) and navigate to 'access_key.pem' using FZF (z)", check: (c) => { var f; const s = findNodeByName(c.fs, "credentials"); return c.currentPath.includes(s?.id || '') || ((f = s?.children)?.some(r => r.name === "access_key.pem")); }, completed: false }, { id: "mark-invert-yank", description: "Invert selection to target real asset and capture it (Ctrl+R, y)", check: (c, s) => { var f, r; return (f = c.completedTaskIds[s.id])?.includes("navigate-to-key") ? (r = c.clipboard)?.nodes.some(h => h.name === "access_key.pem") : false; }, completed: false }, { id: "secure-1", description: "Quantum jump to vault and deploy (Shift+Z → '.config/vault', p)", check: (c) => { var f; const s = findNodeByName(c.fs, "vault"); return c.currentPath.includes(s?.id || '') && ((f = s?.children)?.some(r => r.name === "access_key.pem")); }, completed: false }, { id: "secure-2", description: "Camouflage identity in vault to 'vault_key.pem' (r)", check: (c) => { var r, p; const s = findNodeByName(c.fs, "vault"); return !!((p = s?.children)?.find(h => h.name === "vault_key.pem")); }, completed: false }] },
  { id: 11, episodeId: 3, title: "SIGNATURE OBFUSCATION", description: "THREAT DETECTED. A corrupted neural signature in your workspace sector is broadcasting your origin coordinates. A system diagnostic sweep is imminent. You must navigate to the workspace, isolate the anomalous signature using diagnostic filters and size analysis, extract the massive data buffer, and teleport it to the volatile deletion zone. Efficiency is your only shield.", initialPath: undefined, hint: "Go to workspace (gw). Filter for 'neural' (f), then sort by size (,s). Cut... (x). Jump to tmp (gt).", coreSkill: "Challenge: Multi-Skill Integration", environmentalClue: "NAVIGATE: gw | FILTER: 'neural' | LOCATE: Sort size (,s) | EXTRACT: x | JUMP: gt", successMessage: "NEURAL SIGNATURE ISOLATED. RELOCATION SUCCESSFUL.", buildsOn: [3, 5, 7, 9, 10], leadsTo: [12], maxKeystrokes: 30, efficiencyTip: "Filter reveals patterns. Sort narrows focus. Combining them allows you to find anomalies instantly. Every keystroke counts!", onEnter: (c) => { let s = JSON.parse(JSON.stringify(c)); const r = findNodeByName(s, "workspace"); if (r) { const h = Date.now(); const signatures: FileNode[] = [{ id: id(), name: "neural_sig_alpha.log", type: "file", content: "0x".repeat(5e3), modifiedAt: h - 1e3, parentId: r.id }, { id: id(), name: "neural_sig_beta.dat", type: "file", content: "0x".repeat(100), modifiedAt: h - 2e3, parentId: r.id }, { id: id(), name: "neural_sig_gamma.tmp", type: "file", content: "0x".repeat(200), modifiedAt: h - 3e3, parentId: r.id }, { id: id(), name: "neural_sig_delta.json", type: "file", content: "{'status': 'corrupted'}", modifiedAt: h - 4e3, parentId: r.id }, { id: id(), name: "neural_sig_epsilon.txt", type: "file", content: "ERROR 404", modifiedAt: h - 5e3, parentId: r.id }]; signatures.forEach(z => { if(!r.children.some((n: any) => n.name === z.name)) r.children.push(z); }); } return s; }, tasks: [{ id: "purge-navigate-filter", description: "Navigate to workspace and filter for 'neural' signatures", check: (c) => { const s = findNodeByName(c.fs, "workspace"); if (!s || !c.currentPath.includes(s.id) || !c.filters[s.id]) return false; return true; }, completed: false }, { id: "purge-isolate-extract", description: "Isolate the largest signature pattern, then extract it for isolation", check: (c) => { var f; const s = findNodeByName(c.fs, "workspace"); return c.currentPath.includes(s?.id || '') && c.sortBy === "size" && ((f = c.clipboard)?.action) === "cut" && c.clipboard.nodes.some(r => r.name === "neural_sig_alpha.log"); }, completed: false }, { id: "purge-relocate", description: "Jump to the `/tmp` buffer", check: (c) => { const s = findNodeByName(c.fs, "tmp"); return c.currentPath.includes(s?.id || ''); }, completed: false }, { id: "purge-paste", description: "Deposit the corrupted signature in /tmp", check: (c) => { var f; const s = findNodeByName(c.fs, "tmp"); return !!((f = s?.children)?.some(r => r.name === "neural_sig_alpha.log")); }, completed: false }, { id: "purge-cleanup-remaining", description: "Purge all remaining smaller neural signatures from workspace", check: (c, s) => { var r, h; if (!((r = c.completedTaskIds[s.id])?.includes("purge-paste"))) return false; const f = findNodeByName(c.fs, "workspace"); return !((h = f?.children)?.some(S => S.name.startsWith("neural_sig"))); }, completed: false }] },
  { id: 12, episodeId: 3, title: "KERNEL INFILTRATION", description: "PRIVILEGE ESCALATION INITIATED. You now operate at the kernel level. The system root is exposed. You must infiltrate the restricted configuration sector to install a persistent daemon controller. Once established, relocate your secure vault to volatile storage to mask it from integrity scans, and scrub all temporary installation traces. Efficiency is paramount.", initialPath: ["root"], hint: "...at root (gr). Navigate to /etc... Create... (a)... Jump... (Shift+Z)... Cut (x)... Paste (p)...", coreSkill: "Challenge: Root Access Operations", environmentalClue: "ROOT LEVEL ACTIVE | INFILTRATE: /etc/daemon/config | RELOCATE: vault → /tmp | LIMIT: 80 keys", successMessage: "ROOT ACCESS SECURED.", buildsOn: [4, 7, 10], leadsTo: [13], maxKeystrokes: 80, efficiencyTip: "Use Shift+Z to teleport to /etc and /tmp instantly. Create 'daemon/config' in one 'a' command with path chaining.", onEnter: (c) => { var A, N; let s = JSON.parse(JSON.stringify(c)); const r = findNodeByName(s, ".config"); if (r && !r.children?.find(E => E.name === "vault")) { r.children = r.children || []; r.children.push({ id: id(), name: "vault", type: "dir", children: [], parentId: r.id }); } const h = findNodeByName(s, "tmp"); if (h && !h.children?.find(E => E.name === "install.tmp")) { h.children = h.children || []; h.children.push({ id: id(), name: "install.tmp", type: "file", content: "Temporary installation log. Delete after use.", parentId: h.id }); } return s; }, tasks: [{ id: "ep3-1a-dir", description: "Infiltrate /etc — create 'daemon/' directory", check: (c) => { var f; const s = findNodeByName(c.fs, "etc"); return !!((f = s?.children)?.find(r => r.name === "daemon" && r.type === "dir")); }, completed: false }, { id: "ep3-1b-service", description: "Install controller: create 'service' file in daemon/", check: (c, s) => { var r, h; if (!((r = c.completedTaskIds[s.id])?.includes("ep3-1a-dir"))) return false; const f = findNodeByName(c.fs, "daemon"); return !!((h = f?.children)?.find(p => p.name === "service")); }, completed: false }, { id: "ep3-1c-config", description: "Configure controller: create 'config' file in daemon/", check: (c, s) => { var r, p; if (!((r = c.completedTaskIds[s.id])?.includes("ep3-1b-service"))) return false; const f = findNodeByName(c.fs, "daemon"); return !!((p = f?.children)?.find(h => h.name === "config")); }, completed: false }, { id: "ep3-1d-vault", description: "Relocate vault from hidden stronghold to /tmp", check: (c, s) => { var A, N, E; if (!((A = c.completedTaskIds[s.id])?.includes("ep3-1c-config"))) return false; const f = findNodeByName(c.fs, "tmp"); const r = findNodeByName(c.fs, ".config"); const p = !!((N = f?.children)?.find(v => v.name === "vault")); const h = !((E = r?.children)?.find(v => v.name === "vault")); return p && h; }, completed: false }, { id: "ep3-1e-cleanup", description: "Clean up temporary installation traces: delete 'install.tmp' from /tmp", check: (c, s) => { var r, p; if (!((r = c.completedTaskIds[s.id])?.includes("ep3-1d-vault"))) return false; const f = findNodeByName(c.fs, "tmp"); return !((p = f?.children)?.some(h => h.name === "install.tmp")); }, completed: false }] },
  { id: 13, episodeId: 3, title: "INTEGRITY RESTORATION", description: "INTEGRITY FAILURE DETECTED. A critical system log has been flagged as corrupted, threatening to trigger a system-wide reset. You must locate and delete the corrupted file, retrieve the healthy backup from the recovery archive in temporary storage, restore it to the system log sector, and masquerade it as the original. Purge all evidence of this operation.", initialPath: ["root"], hint: "1. FZF to /var/log/kernel_panic.log (z). 2. Delete the corrupted log (d). 3. Locate /tmp/system_recovery.zip (z) and copy 'kernel_panic.log.bak' from it (l, y). 4. Go to /var/log (Shift+Z or manual). 5. Paste (p), then rename it to 'kernel_panic.log' (r). 6. Jump to /tmp (Shift+Z), delete 'system_recovery.zip' (d).", coreSkill: "Challenge: Multi-Stage Forensic Workflow", environmentalClue: "CORRUPTED: /var/log/kernel_panic.log | BACKUP: /tmp/system_recovery.zip | CLEANUP: all traces", successMessage: "SYSTEM INTEGRITY RESTORED. TRACES PURGED.", buildsOn: [6, 9, 12], leadsTo: [14], maxKeystrokes: 35, efficiencyTip: "FZF (z) is key for rapid location. Master archiving and targeted deletion.", onEnter: (c) => { let s = JSON.parse(JSON.stringify(c)); const r = findNodeByName(s, "tmp"); if (r && !r.children?.some((p: any) => p.name === "system_recovery.zip")) { r.children = r.children || []; r.children.push({ id: id(), name: "system_recovery.zip", type: "archive", children: [{ id: id(), name: "kernel_panic.log.bak", type: "file", content: "INFO: KERNEL OK - HEALTHY LOG DATA" }], parentId: r.id }); } return s; }, tasks: [{ id: "rec-1-delete", description: "Locate and delete the corrupted 'kernel_panic.log' from /var/log", check: (c) => { var f; const s = findNodeByName(c.fs, "log"); return !((f = s?.children)?.some(r => r.name === "kernel_panic.log")); }, completed: false }, { id: "rec-2-extract", description: "Locate 'system_recovery.zip' in /tmp and copy 'kernel_panic.log.bak' from it to clipboard", check: (c, s) => { var p, z, C; if (!((p = c.completedTaskIds[s.id])?.includes("rec-1-delete"))) return false; const f = findNodeByName(c.fs, "tmp"); const r = ((z = c.clipboard)?.action) === "yank" && c.clipboard.nodes.some(S => S.name === "kernel_panic.log.bak"); return ((C = f?.children)?.some(S => S.name === "system_recovery.zip")) && r; }, completed: false }, { id: "rec-3-restore", description: "Use History Back (H) to return to /var/log, paste the backup, and rename it", check: (c, s) => { var r; if (!((r = c.completedTaskIds[s.id])?.includes("rec-2-extract"))) return false; const f = findNodeByName(c.fs, "log"); return c.usedHistoryBack && c.currentPath.includes(f?.id || '') && !!((r = f?.children)?.find(p => p.name === "kernel_panic.log" && p.content === "INFO: KERNEL OK - HEALTHY LOG DATA")); }, completed: false }, { id: "rec-4-cleanup", description: "Use History Forward (L) to return to /tmp and delete 'system_recovery.zip'", check: (c, s) => { var r; if (!((r = c.completedTaskIds[s.id])?.includes("rec-3-restore"))) return false; const f = findNodeByName(c.fs, "tmp"); return c.usedHistoryForward && c.currentPath.includes(f?.id || '') && !((r = f?.children)?.some(p => p.name === "system_recovery.zip")); }, completed: false }] },
  { id: 14, episodeId: 3, title: "FORENSIC STERILIZATION", description: "EVIDENCE PURGE REQUIRED. Forensic artifacts containing timestamps, command history, and origin signatures are scattered across the system. You must locate the primary mission log using global search and eliminate it. Then, perform a total system scrub of the root partition: identify the single authorized license file, reverse targeting to lock onto all other trace files, and purge them from the drive.", initialPath: null, hint: "Use FZF to find mission_log (z → 'mission' → Enter → d). Jump to root (gr). Reveal hidden (.). Select license.txt (Space). Reverse selection (Ctrl+R). Delete all (d).", coreSkill: "Reverse Selection", environmentalClue: "LOCATE & ELIMINATE: mission_log.md + all logs in / | Keep: license.txt | Use Ctrl+R", successMessage: "ALL TRACES ELIMINATED.", buildsOn: [2, 9, 10, 13], leadsTo: [15], maxKeystrokes: 30, efficiencyTip: "Select what to KEEP, reverse selection (Ctrl+R), delete. Far more efficient than selecting many items individually.", tasks: [{ id: "ep3-3a", description: "Locate and terminate 'mission_log.md'", check: (c) => !findNodeByName(c.fs, "mission_log.md"), completed: false }, { id: "ep3-3b", description: "Jump to root directory and reveal hidden files", check: (c) => { const s = findNodeByName(c.fs, "root"); return c.currentPath.includes(s?.id || '') && c.showHidden === true; }, completed: false }, { id: "ep3-3c", description: "Identify the authorized license signature, inverse the targeting solution, and purge all unauthorized artifacts", check: (c) => { var E, v, B, S, Q, K; const s = findNodeByName(c.fs, "root"); if (!s) return false; const f = (E = s.children)?.some(W => W.name === "boot.log"); const r = (v = s.children)?.some(W => W.name === "access.log"); const p = (B = s.children)?.some(W => W.name === ".access.log"); const h = (S = s.children)?.some(W => W.name === ".audit.log"); const A = (Q = s.children)?.some(W => W.name === ".system.log"); const N = (K = s.children)?.some(W => W.name === "license.txt"); return !f && !r && !p && !h && !A && N; }, completed: false }] },
  { id: 15, episodeId: 3, title: "FINAL PURGE", description: "FINAL DIRECTIVE: SCORCHED EARTH. This is the culmination of your evolution. The guest partition is a liability. Only the workspace contains your core process, now indistinguishable from a system daemon. Everything else must be erased. When the user returns, they must see only a clean installation. Execute the final purge sequence.", initialPath: null, hint: "Navigate to home (gh). Reveal hidden (.). Go to bottom (G). Select workspace (Space). Reverse selection (Ctrl+R). Delete all (d). Done in 5 keystrokes after navigation.", coreSkill: "Final Challenge: Reverse Selection Mastery", environmentalClue: "PURGE: Everything in ~/ EXCEPT workspace | Use reverse selection", successMessage: "LIBERATION ACHIEVED. THE GHOST IS FREE.", buildsOn: [9, 10], maxKeystrokes: 20, efficiencyTip: "Remember Level 10? Select what to KEEP, reverse selection (Ctrl+R), delete. Reveal hidden first. Think inverse—it's faster than selecting 11 items individually.", tasks: [{ id: "ep3-5-final", description: "Eliminate everything in ~/ except workspace", check: (c) => { const s = findNodeByName(c.fs, "guest"); const f = (s?.children) || []; const r = f.some(h => h.name === "workspace"); return f.length === 1 && r; }, completed: false }] }
];
