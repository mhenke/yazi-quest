...
1.  **Paste Overwrite Behavior:**
    ...
    - **Yazi Quest:** Halts a standard paste (`p`) operation and prompts for confirmation if a file with the same name exists. However, the `P` (force overwrite) command IS implemented and will overwrite the destination file without confirmation, providing a parallel to Yazi's non-interactive overwrite capabilities. This difference in the standard paste is a deliberate pedagogical choice.
